...
<script lang="ts">
	...
	async function load() {
		const res = await api.get(`/fund/collections/${orderId}`);
		order = res;
	}
...
<div class="bg-base-100 flex h-full flex-col">
	<div class="border-base-300 flex items-center gap-4 border-b p-4 print:hidden">
		...
		>
		<h1 class="text-xl font-bold flex-1">收款单详情：{order?.statement_no || '-'}</h1>
	...
	{#if order}
		<div class="space-y-6 p-6">
			<div class="grid grid-cols-4 gap-4">
				<div>
					<span class="text-base-content/60 text-sm">客户：</span><br /><b>{order.customer_name}</b>
				</div>
				<div>
				...
				<div>
					<span class="text-base-content/60 text-sm">销售/抵充：</span><br /><b
						class="text-success font-mono">¥{order.collection_amount}</b
					>
				</div>
				<div>
				...
				<div>
					<span class="text-base-content/60 text-sm">实际收款：</span><br /><b
						class="text-success font-mono">¥{order.actual_amount}</b
					>
				</div>
				<div>
					<span class="text-base-content/60 text-sm">销售与实收差额：</span><br /><b

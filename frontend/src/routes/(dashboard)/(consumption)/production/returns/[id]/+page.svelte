...
<script lang="ts">
	...
	async function loadRelated() {
		try {
			const coRes: any = await api.get(`/production/returns/${id()}/consumption-orders`);
			consumptionOrders = coRes || [];
		} catch (e) {
			console.error(e);
		}
		try {
			const roRes: any = await api.get(`/production/returns/${id()}/reversal-orders`);
	...
	async function handleSave() {
		saving = true;
		try {
			await api.put(`/production/returns/${id()}`, {
	...
	function statusName(v: string) {
		const map: any = { created: '待确认', confirmed: '已确认', completed: '已完成', cancelled: '已取消' };
		return map[v] || v || '-';
	}
...
<div class="flex min-h-0 flex-1 flex-col gap-4">
	<div class="flex items-center justify-between gap-2 print:hidden">
		<div class="flex items-center gap-2">
			<button class="btn btn-ghost gap-2" onclick={() => goto('/production/returns')}>
				<ArrowLeft size={18} /> 返回
			</button>
			<h1 class="text-xl font-bold">生产退货单详情</h1>
		</div>
		{#if data}
	...
	{:else}
		<div class="card bg-base-100 border-base-300 border">
			<div class="card-body grid grid-cols-1 gap-4 md:grid-cols-2">
				<div>
					<div class="text-base-content/60 text-sm">退货单号</div>
					<div class="font-mono text-lg">{fmt(data.return_no)}</div>
				</div>
				<div>
					<div class="text-base-content/60 text-sm">状态</div>
					<div class="badge">{statusName(data.status)}</div>
				</div>

				<div>
					<div class="text-base-content/60 text-sm">关联生产单</div>
					{#if data.production_order_id}
						<a class="link link-primary font-mono" href={`/production/orders/${data.production_order_id}`}>
							{fmt(data.production_no)}</a>
					{:else}
				...
				<div>
					<div class="text-base-content/60 text-sm">退回数量</div>
					<div class="font-mono">{Number(data.returned_quantity || 0).toFixed(3)}</div>
				</div>
				...
				<div class="md:col-span-2">
					...
					{#if showEdit}
						<div class="flex items-center gap-2 mt-1">
							<input type="number" bind:value={editCostPrice} min="0" step="0.01" class="input input-bordered h-10 w-48 text-base font-mono" />
					...
					{:else}
						<div class="font-mono text-lg font-semibold">¥{fmtMoney(data.cost_price)}</div>
					{/if}
				</div>
						...
						<table class="table-zebra table w-full text-sm">
							...
							<tbody>
								{#each consumptionOrders as co}
									<tr>
										<td><a class="link link-primary font-mono" href={`/consumption/orders/${co.id}`}>{co.order_no}</a></td>
										...
										<td><span class="badge badge-sm">{co.status_name || co.status}</span></td>
						...
						<table class="table-zebra table w-full text-sm">
							...
							<tbody>
								{#each reversalOrders as ro}
									<tr>
										<td><a class="link link-primary font-mono" href={`/reversal/orders/${ro.id}`}>{ro.order_no}</a></td>
										...
										<td><span class="badge badge-sm">{ro.status_name || ro.status}</span></td>

<script lang="ts">
	...
	async function loadDetail(id: number) {
		loading = true;
		try {
			order = await api.get(`/consumption/orders/${id}`);
		} catch (err: any) {
			toast.error('加载领料单详情失败: ' + (err?.message || err));
			order = null;
		} finally {
			loading = false;
		}
...
<div class="space-y-6 text-base">
	<div class="flex items-center justify-between">
		<div class="flex items-center gap-3">
			...
			<h1 class="text-2xl font-bold tracking-tight">领料单详情</h1>
		</div>
		...
		<div class="breadcrumbs text-base opacity-60">
			<ul>
				<li>首页</li>
				<li>出入库管理</li>
				<li><a class="text-primary" href="/consumption/orders">领料</a></li>
				<li>详情</li>
			</ul>
		</div>
	</div>
	...
	{:else}
		<div class="bg-base-100 border-base-300 space-y-5 rounded-2xl border p-5">
			...
			<div class="grid grid-cols-1 gap-4 lg:grid-cols-4">
				...
				<div class="bg-base-200/40 border-base-300 rounded-xl border p-4">
					<div class="text-base-content/55 text-sm">领料金额合计</div>
				...
				<div class="bg-base-200/40 border-base-300 rounded-xl border p-4">
					<div class="text-base-content/55 text-sm">关联出库单号</div>
					<div class="mt-1 text-base">
		...
		<div class="bg-base-100 border-base-300 space-y-4 rounded-2xl border p-5">
			<div class="flex items-center justify-between gap-3">
				<div class="text-lg font-semibold">领料明细</div>
			...
			<div class="overflow-x-auto">
				<table class="table-zebra table w-full min-w-[980px] table-fixed text-base">
					<thead>
						<tr>
							<th class="w-[46%] min-w-[320px]">物料</th>
							<th class="w-24 whitespace-nowrap">属性</th>
							<th class="text-right">数量</th>
							<th class="min-w-[5rem]">单位</th>
							<th class="min-w-[7rem] text-right">单价</th>
							<th class="min-w-[8rem] text-right">金额</th>
						</tr>
					</thead>
					<tbody>

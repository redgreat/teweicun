...
func ListInventoryChecks(c *gin.Context) {
	var q request.InventoryCheckQuery
	...
	list, total, err := service.ListInventoryChecks(c.Request.Context(), &q)
	if err != nil {
...
func GetInventoryCheckDetail(c *gin.Context) {
	...
	detail, err := service.GetInventoryCheckDetail(c.Request.Context(), id)
	if err != nil {
		response.Error(c, err)
		return
	}
...
func CreateInventoryCheck(c *gin.Context) {
	var req request.CreateInventoryCheckReq
	...
	id, err := service.CreateInventoryCheck(c.Request.Context(), &req, userID, username)
	if err != nil {
		response.Error(c, err)
		return
	}
...
func ConfirmInventoryCheck(c *gin.Context) {
	...
	err = service.ConfirmInventoryCheck(c.Request.Context(), id, userID, username)
	if err != nil {
		response.Error(c, err)
		return
	}
